    ...
    async invokeChaincode(peerNames, channelName, chaincodeName, fcn, args, orgName, userName) {
        ...
        try {
            ...
            if (all_good) {
                ...
                logger.info("Endored by:", proposalResponses[0].peer.name)
                ...
                var resultsPromise = await Promise.all(promises.map(promise => promise.catch(e => e)));
                ...
                logger.info('Number of committed peers: ', committedCount)
                if (resultsPromise.length > 0 && committedCount == 0) {
                    error_message = "There aren't any peer in the configration committed the transaction."
                }
            }
        } catch (error) {
